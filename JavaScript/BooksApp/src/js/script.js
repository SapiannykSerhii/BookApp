...
const templates = {
  bookTemplate: Handlebars.compile(document.getElementById('template-book').innerHTML),
};
class BooksList{
  constructor(){
    const thisBook = this;
  ...
  filterBooks(){
    ...
    for(let bookFilt of dataSource.books){
      ...
      if(shouldBeHidden){
        document.querySelector(`.book__image[data-id="${bookFilt.id}"]`).classList.add('hidden');
      } else {
        document.querySelector(`.book__image[data-id="${bookFilt.id}"]`).classList.remove('hidden');
      }
    }
  }
